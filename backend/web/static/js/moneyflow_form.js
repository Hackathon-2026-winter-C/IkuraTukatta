...
const syncHiddenCategoryFromSelected = () => {
  ...
  if (hiddenCategoryId) {
    const hasVisibleMatch = Array.from(picks || []).some((btn) => btn.dataset.id === hiddenCategoryId && !btn.classList.contains("hidden"));
    if (hasVisibleMatch) return hiddenCategoryId;
    categoryHidden.value = "";
  }

  const selected = document.querySelector(".cat-pick.is-selected:not(.hidden)");
...
const modeHidden = document.getElementById("record-mode-hidden");
const receiptOpenBtn = document.getElementById("receipt-open-btn");

const applyModeToCategories = (mode) => {
  if (!picks.length) return;

  const categoryMode = mode === "income" ? "income" : "expense";
  picks.forEach((btn) => {
    const btnMode = btn.dataset.mode || "expense";
    const shouldShow = btnMode === categoryMode;
    btn.classList.toggle("hidden", !shouldShow);
    if (!shouldShow) btn.classList.remove("is-selected");
  });
...
const syncReceiptButton = (mode) => {
  if (!receiptOpenBtn) return;
  receiptOpenBtn.classList.toggle("hidden", mode !== "receipt");
};
...
document.querySelectorAll('input[name="record-mode"]').forEach((radio) => {
  radio.addEventListener("change", () => {
    ...
    syncReceiptButton(mode);
    applyModeToCategories(mode);
  });
});
...
const deleteCloseBtns = deleteModal?.querySelectorAll("[data-close-moneyflow-delete-modal]") || [];
